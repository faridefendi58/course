...
class Admin extends CI_Controller {
    ...
    public function invoices($param1 = "", $param2 = "") {
        ...
        elseif ($param1 == "mark_as_paid") {
            $this->invoice_model->mark_as_paid($param2);
            redirect(site_url('admin/invoices/view/1'), 'refresh');
        }

        $this->session->set_userdata('last_page', 'invoices');
        $page_data['page_name'] = 'invoices';
        $page_data['page_title'] = get_phrase('invoices');
        $page_data['invoices'] = $this->invoice_model->get_invoices_by_status($param2);
        $this->load->view('backend/index', $page_data);
    }

    public function invoice_form($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == 'add_invoice_form') {
            $page_data['page_name'] = 'user_add';
            $page_data['page_title'] = get_phrase('invoice_add');
            $this->load->view('backend/index', $page_data);
        }
        elseif ($param1 == 'edit_invoice_form') {
            $page_data['page_name'] = 'invoice_edit';
            $page_data['user_id'] = $param2;
            $page_data['page_title'] = get_phrase('invoice_edit');
            $this->load->view('backend/index', $page_data);
        }
    }
    /** endof invoices */
    ...
    public function orders($param1 = "", $param2 = "") {
        ...
        $model = $this->order_model->get_order($param2)->row();

        if ($param1 == "detail") {
            $this->session->set_userdata('last_page', 'orders');
            $page_data['page_name'] = 'order_detail';
            $page_data['page_title'] = get_phrase('order_detail');
            $page_data['model'] = $model;
            $this->load->view('backend/index', $page_data);
            return false;
        }
        elseif ($param1 == "edit") {
            $this->order_model->edit_order($param2);
        ...
        elseif ($param1 == "suspend") {
            $this->order_model->suspend($param2);
            redirect(site_url('admin/orders/detail/'. $param2), 'refresh');
        }
        elseif ($param1 == "unsuspend") {
            $this->order_model->unsuspend($param2);
            redirect(site_url('admin/orders/detail/'. $param2), 'refresh');
        }
        elseif ($param1 == "complete") {
            $this->order_model->complete($param2);
            redirect(site_url('admin/orders/detail/'. $param2), 'refresh');
        }
