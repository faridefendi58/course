...
class Home extends CI_Controller {
    ...
    /**
     * Checkout with transfer bank
     */
    public function transfer_bank_checkout() {
        if ($this->session->userdata('user_login') != 1)
            redirect('home', 'refresh');

        $currency = $this->crud_model->get_currency_by_code(get_settings('system_currency'));
        $params = [];
        if (is_object($currency)) {
            $params['currency_id'] = $currency->id;
            $params['change_value'] = 1;
        }

        $cart_items = [];
        if (count($this->session->get_userdata('cart_items')) > 0) {
            foreach ($this->session->userdata('cart_items') as $i => $course_id) {
                $course = $this->crud_model->get_course_by_id($course_id)->row();
                $cart_data = [
                    'course_id' => $course_id,
                    'title' => $course->title,
                    'price' => $course->price,
                ...
                $cart_items[$course_id] = $cart_data;
            }
        }
