...
class Crud_model extends CI_Model {
    ...
    public function update_phrase($data) {
        $this->db->where('phrase_id', $data['phrase_id']);
        $this->db->update('language', $data);
    }

    public function get_status_wise_invoices($status = "") {
        if ($status != "") {
            $courses = $this->db->get_where('invoice', array('status' => $status));
        }else {
            $courses['unpaid'] = $this->db->get_where('invoice', array('status' => 0));
            $courses['paid'] = $this->db->get_where('invoice', array('status' => 1));
            $courses['refunded'] = $this->db->get_where('invoice', array('status' => 2));
        }
    ...
    /**
     * Getting user already enrolled
     * @param $course_id
     * @param $user_id
     * @return bool
     */
    public function is_already_enrolled($course_id, $user_id) {
